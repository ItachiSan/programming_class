...
#include <stdio.h>
#include <stdlib.h>
// Useful for some string operations
#include <string.h>
// Boolean
#include <stdbool.h>
// Getting error information
#include <errno.h>
// For 'waitpid' and related stuff
#include <sys/wait.h>
...
int echo_service(int active_socket_descriptor) {
	...
	int result;
	// Boolean representing if we are running or not
	bool running = true;
	...
	printf("server: closing child process for client %s:%d\n",
		inet_ntoa(client_socket_address.sin_addr),
		ntohs(client_socket_address.sin_port)
	);
	return result;
...
// Handle SIGCHLD signals (child processes returns) with this handy function
void handle_sigchild() {
	// Exit status of children
	int status;
	// Wait a child to end
	while(waitpid(-1, &status, WNOHANG) > 0);
	// Re-register for another child process
...
int server_mode() {
	...
	// Get properely the maximum number of sockets we can use
	...
	// Be ready for child process!
	signal(SIGCHLD, handle_sigchild);

	// Accept connections, even multiple ones
	int active_socket_descriptor;
	while (running) {
		...
		// Fork; parent process will keep in this loop, the child in the echo one
		switch(fork()) {
			// We are the child process, let's run the echo service
			case 0:
				close(socket_descriptor);
				return echo_service(active_socket_descriptor);
			case -1:
				fprintf(stderr, "server: error creating a child process (%s)", strerror(errno));
				return EXIT_FAILURE;
			// We are the father process, keep running and ignore the active socket
			default:
				close(active_socket_descriptor);
		}
	}
	// Closing the server
